...
type CollabOp = SortableOp | DragOp | ResizeOp | RotateOp | SplitPaneOp | PanZoomOp;
/** In-flight presence as emitted locally (the Room stamps `peerId` on the wire). */
type LocalPresence = {
  type: 'sortable'; /** The hovered list (gap opens here). */
  target: string; /** The list the drag started in. */
  fromTarget: string;
  itemId: string;
  insertIndex: number;
  rel: {
  ...
  type: 'drop-hover'; /** The hovered zone's id. */
  target: string; /** The peer's cursor in viewport coords. */
  x: number;
  y: number; /** Best-effort identity of what's being dragged (e.g. a `data-neodrag-sortable-key`), for a label. */
  itemId?: string;
};
/** A presence frame on the wire / arriving from a remote peer. */
type PresenceFrame = LocalPresence & {
  peerId: string;
};
/**
 ...
 */
type Mirror = boolean | HTMLElement;
/**
...
interface CollabTarget {
  ...
  clearRemotePresence(peerId?: string, opts?: {
    ease?: boolean;
  }): void;
...
//#endregion
export { Mirror as a, RotateOp as c, LocalPresence as i, SortableOp as l, CollabTarget as n, PresenceFrame as o, DragOp as r, ResizeOp as s, CollabOp as t };